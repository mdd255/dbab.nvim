...
describe("adapter", function()
	describe("parse_url", function()
		...
		describe("redis", function()
			it("parses full redis URL", function()
				local result = adapter.parse_url("redis://user:secret@localhost:6379/0")
				assert.are.equal("redis", result.scheme)
				assert.are.equal("user", result.user)
				assert.are.equal("secret", result.password)
				assert.are.equal("localhost", result.host)
				assert.are.equal("6379", result.port)
				assert.are.equal("0", result.database)
			end)

			it("parses redis URL without auth", function()
				local result = adapter.parse_url("redis://localhost:6379/0")
				assert.are.equal("redis", result.scheme)
				assert.is_nil(result.user)
				assert.are.equal("localhost", result.host)
				assert.are.equal("6379", result.port)
			...
			it("parses redis URL without database", function()
				local result = adapter.parse_url("redis://localhost/0")
				assert.are.equal("redis", result.scheme)
				assert.are.equal("localhost", result.host)
				assert.are.equal("0", result.database)
			end)
	...
	describe("build_cmd", function()
		...
		describe("mysql", function()
			it("returns mysql with parsed args", function()
				local cmd, args, env = adapter.build_cmd("mysql://user:pass@localhost:3306/mydb")
				...
				-- Password goes via MYSQL_PWD env, never argv (no leak in `ps`).
				assert.is_nil(arg_str:find("pass"))
				assert.are.equal("pass", env.MYSQL_PWD)
			end)
		...
		describe("redis", function()
			it("returns redis-cli with parsed args", function()
				local cmd, args, env = adapter.build_cmd("redis://user:secret@localhost:6379/2")
				assert.are.equal("redis-cli", cmd)

				local arg_str = table.concat(args, " ")
				assert.is_true(arg_str:find("-h") ~= nil)
				assert.is_true(arg_str:find("localhost") ~= nil)
				assert.is_true(arg_str:find("-p") ~= nil)
				assert.is_true(arg_str:find("6379") ~= nil)
				assert.is_true(arg_str:find("--no%-auth%-warning") ~= nil)
				assert.is_true(arg_str:find("-n") ~= nil)
				assert.is_true(arg_str:find("2") ~= nil)
				-- Password goes via REDISCLI_AUTH env, never argv (no leak in `ps`).
				assert.is_nil(arg_str:find("secret"))
				assert.are.equal("secret", env.REDISCLI_AUTH)
			end)

			it("builds redis-cli without optional fields", function()
				local cmd, args = adapter.build_cmd("redis://localhost")
				assert.are.equal("redis-cli", cmd)
				local arg_str = table.concat(args, " ")
				assert.is_true(arg_str:find("localhost") ~= nil)
				assert.is_nil(arg_str:find("-a"))
				assert.is_nil(arg_str:find("-n"))
			end)
		end)

		it("errors on unsupported database type", function()
			assert.has_error(function()
				adapter.build_cmd("cassandra://localhost/mydb")
			end)
		end)
	end)
end)
