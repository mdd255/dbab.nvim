...
--- Wrap a rendered style's output lines into a Dbab.QueryResult: each line becomes
--- a single-cell row so the grid can display pre-formatted text uniformly.
---@param raw string
---@param columns string[]
---@param lines string[]
---@param row_count number
---@return Dbab.QueryResult
local function wrap_lines(raw, columns, lines, row_count)
	local rows = {}
	...
	return { raw = raw, columns = columns, rows = rows, row_count = row_count }
end

---@param table_result Dbab.QueryResult
---@return string[] lines, number encoded_count
local function render_json(table_result)
	...
	for idx, item_str in ipairs(encoded) do
		table.insert(lines, "  " .. item_str .. (idx < #encoded and "," or ""))
	end
...
---@param table_result Dbab.QueryResult
---@return string[]
local function render_vertical(table_result)
	local col_width = 0
...
---@param table_result Dbab.QueryResult
---@return string[]
local function render_markdown(table_result)
	...
	for i, col in ipairs(table_result.columns) do
		...
		table.insert(sep_parts, string.rep("-", widths[i] + 2))
	end

	local lines = {
		"|" .. table.concat(header_parts, "|") .. "|",
		"|" .. table.concat(sep_parts, "|") .. "|",
	}
...
function M.parse(raw, style)
	if style == "raw" then
		...
		return wrap_lines(raw, { "raw" }, lines, #lines)
	end

	local table_result = M.parse_table(raw)

	if style == "json" then
		local lines, row_count = render_json(table_result)
		return wrap_lines(table.concat(lines, "\n"), table_result.columns, lines, row_count)
	end

	if style == "vertical" then
		local lines = render_vertical(table_result)
		return wrap_lines(table.concat(lines, "\n"), table_result.columns, lines, #table_result.rows)
	end

	if style == "markdown" then
		local lines = render_markdown(table_result)
		return wrap_lines(table.concat(lines, "\n"), table_result.columns, lines, #table_result.rows)
	end
