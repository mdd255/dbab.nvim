	...
	for _, conn in ipairs(connections) do
		...
		if is_expanded and conn_url then
			...
			do
				...
				if schema_db_type == "mysql" or schema_db_type == "sqlite" or schema_db_type == "mongodb" or schema_db_type == "redis" then
					...
					if schemas_expanded then
						for _, sch in ipairs(db_schemas) do
							...
							table.insert(
								lines,
								pad_right(schema_text, sidebar_width - #schema_suffix - 1, schema_suffix)
							)
							table.insert(M.nodes, {
								type = "schema",
								name = sch.name,
...
function M.insert_table_query()
	...
	if node.type == "table" or node.type == "view" then
		...
		if db_type == "redis" then
			return string.format("GET %s", node.name)
		end
