...
function M.refresh_result_winbar()
    ...
    if M.last_query then
        ...
        if M.last_result and M.last_result.row_count then
            table.insert(
                suffix_parts,
                "%#DbabSidebarIconTable#" .. icons.rows .. "%* %#DbabNumber#" .. M.last_result.row_count .. " rows%*"
            )
        ...
        if M.last_duration then
            table.insert(
                suffix_parts,
                "%#Comment#" .. icons.duration .. " " .. format_duration(M.last_duration) .. "%*"
            )
...
local function render_tabbar()
    ...
    for i, tab in ipairs(M.query_tabs) do
        ...
        local icon_hl = is_active and "DbabTabActiveIcon"
            or (tab.is_saved and "DbabTabIconSaved" or "DbabTabIconUnsaved")
        ...
        if #fields > 0 then
            ...
            for _, f in ipairs(fields) do
                ...
                if type(f.value) == "table" then
                    local win_width = M.result_win
                            and vim.api.nvim_win_is_valid(M.result_win)
                            and vim.api.nvim_win_get_width(M.result_win)
                        or 120
    ...
    for _, line in ipairs(raw_lines) do
        ...
        if indent_len == field_indent then
            ...
            if key then
                ...
                local opens_nested = (val:match("^{") or val:match("^%[")) and not (val:match("}$") or val:match("%]$"))
    ...
    if active_db_type == "mongodb" then
        ...
        if result_style == "vertical" then
            ...
            if vert_lines and #vert_lines > 0 then
                ...
                M.last_result =
                    { columns = {}, rows = {}, row_count = record_count, raw = table.concat(vert_lines, "\n") }
                M.refresh_result_winbar()
...
function M.open()
    ...
    delete_existing_buf("[DB]")
    delete_existing_buf("Explorer")
    delete_existing_buf("Result")
...
function M._init_all_components(windows)
    ...
    if windows.result then
        ...
        pcall(vim.api.nvim_buf_set_name, M.result_buf, "Result")
